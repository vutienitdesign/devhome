			...
			while($the_query->have_posts() ) : $the_query->the_post();
				...
				if(!empty($idSmall)){
					...
					$sHtml = '<tr data-id="'.$dataID.'" data-product="'.$idProduct.'" class="data-product product-id-'.$idProduct.' product-temp-'.$idRandom.'" data-type="set">
			                    ...
			                    <td class="amount"><input type="number" class="input-text soluong custom-data-id" value="1" min="1" data-id="'.$dataID.'" data-product="'.$idProduct.'" data-price="'.$product->get_price().'" data-type="set" data-tag="'.$termID.'|'.$dataAll.'"></td>
